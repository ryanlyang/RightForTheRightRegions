...
def train_model(model, dataloaders, dataset_sizes,
                kl_lambda, num_epochs,
                base_lr, classifier_lr, use_attention, num_classes):
            ...
            for batch in dataloaders[phase]:
                ...
                with torch.set_grad_enabled(is_train):
                    ...
                    if use_attention_this_batch and has_masks:
                        ...
                        loss_tuple = compute_loss(outputs, labels, sal_norm, gt_small, kl_lambda, False)
...
def run_single(args, kl_value):
    ...
    print(f"\n=== RUN: kl_lambda={kl_value} ===", flush=True)
    ...
    if save_checkpoints:
        ...
        save_name = f"resnet50_final_kl{int(kl_value)}_{ts}.pth"
    ...
    print(f"[RUN DONE] kl={kl_value} | best_balanced_val_acc={best_score:.4f} "
...
def main():
    ...
    parser.add_argument('--sweep', action='store_true',
                        help='Run the full hyperparameter sweep (kl 100..300 step 20)')
    ...
    if not args.sweep:
        run_single(args, args.kl_lambda)
        return

    kl_values = list(range(100, 301, 20))

    best_overall = (-1.0, None, None, None, None)  # (best_optim, kl, test_acc, per_group, worst_group)
    for kl in kl_values:
        try:
            score, test_acc, per_group, worst_group, _ = run_single(args, kl)
            if score > best_overall[0]:
                best_overall = (score, kl, test_acc, per_group, worst_group)
        except Exception as e:
            print(f"[SWEEP ERROR] kl={kl} -> {e}", flush=True)

    print("\n=== SWEEP COMPLETE ===")
    if best_overall[1] is not None:
        print(f"Best by val Optim Num: optim={best_overall[0]:.4f}, "
              f"kl={best_overall[1]}, "
              f"test_acc={best_overall[2]:.2f}%, "
              f"per_group={best_overall[3]:.2f}%, "
              f"worst_group={best_overall[4]:.2f}%", flush=True)
    else:
